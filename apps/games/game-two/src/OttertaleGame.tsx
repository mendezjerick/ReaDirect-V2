...
import { Graphics, Sprite, AnimatedSprite, TilingSprite, Assets, Texture, Rectangle, TextStyle, Container } from 'pixi.js';
import * as PIXI from 'pixi.js';

// 1. Register Pixi.js classes
extend({ Graphics, Sprite, AnimatedSprite, TilingSprite, Text: PIXI.Text, Container });
...
const MobileButton = ({ onDown, onUp, label }: { onDown: () => void, onUp: () => void, label: string }) => (
  <button
...
const extractFrames = (baseTexture: PIXI.Texture, w: number, h: number, count: number, startX: number = 0, startY: number = 0) => {
  const frames = [];
...
const extractFramesTrimmed = (
  ...
  for (let i = 0; i < count; i++) {
    ...
    frames.push(new Texture({ source: texture.source, frame: new Rectangle(clippedX, clippedY, clippedW, clippedH) }));
  }
  return frames.length > 0 ? frames : [texture];
};

const cropTexture = (texture: PIXI.Texture, x: number, y: number, w: number, h: number) =>
...
const Coin = ({ textures, x, y, collected }: { textures: Texture[], x: number, y: number, collected: boolean, onCollect: () => void }) => {
  const ref = useRef<PIXI.AnimatedSprite>(null);
  useEffect(() => { if (ref.current) ref.current.play(); }, []);
  if (collected) return null;
  return <pixiAnimatedSprite ref={ref} textures={textures} x={x} y={y} anchor={0.5} scale={{ x: 2, y: 2 }} animationSpeed={0.15} />;
};

const Fruit = ({ texture, x, y, collected }: { texture: Texture, x: number, y: number, collected: boolean, onCollect: () => void }) => {
  if (collected) return null;
  return <pixiSprite texture={texture} x={x} y={y} anchor={0.5} scale={{ x: 2.2, y: 2.2 }} />;
};
...
const Slime = ({ textures, slime, onUpdatePosition, isPaused }: { textures: Texture[], slime: { id: number, x: number, y: number, alive: boolean, direction: number, minX: number, maxX: number }, onUpdatePosition: (id: number, newX: number, newDir: number) => void, isPaused: boolean }) => {
  const ref = useRef<PIXI.AnimatedSprite>(null);
  ...
  return <pixiAnimatedSprite ref={ref} textures={textures} x={slime.x} y={slime.y} anchor={0.5} scale={{ x: 2, y: 2 }} animationSpeed={0.1} />;
};

const Player = ({ 
  assets, coins, onCollectCoin, fruits, onCollectFruit, slimes, onHitSlime, onKillSlime, onMove, onReachFinish, resetTrigger, isDead, inputKeys, isPaused, activeBuff,
  stageConfig
}: {
  assets: any,
  ...
  slimes: { id: number, x: number, y: number, alive: boolean, direction: number, minX: number, maxX: number }[],
  onHitSlime: () => void,
  ...
  const physics = useRef({ x: 100, y: 200, velocityX: 0, velocityY: 0, isGrounded: false });
  ...
  useTick(() => {
    if (!spriteRef.current || isDead || isPaused) {
      if (spriteRef.current && spriteRef.current.playing && isPaused) spriteRef.current.stop();
      return;
    }
    ...
    if (nextAnim !== currentAnim.current) {
      spriteRef.current.textures = nextAnim === 'run' ? assets.playerRun : assets.playerIdle;
    ...
    if (p.x > stageConfig.worldWidth - playerRenderSize / 2) p.x = stageConfig.worldWidth - playerRenderSize / 2;
    ...
    if (playerBox.bottom >= floorLevel && p.velocityY >= 0 && playerBox.top < floorLevel) {
    ...
    slimes.forEach(slime => {
      ...
      const slimeKillBox = { left: slime.x - 24, right: slime.x + 24, top: slime.y - 24, bottom: slime.y + SLIME_FOOT_OFFSET };
      const slimeDamageBox = { left: slime.x - 14, right: slime.x + 14, top: slime.y - 10, bottom: slime.y + SLIME_FOOT_OFFSET };
...
const TUTORIAL_STAGE = {
  ...
  fruits: [
    { id: 1, x: 1300, y: 450, collected: false }
  ],
  slimes: [
    { id: 1, x: 2300, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 2100, maxX: 2500, color: 'green' as const },
  ],
  texts: [
...
const STAGES = [
  {
    ...
    fruits: [
      { id: 1, x: 1000, y: 450, collected: false }
    ],
    slimes: [
      { id: 1, x: 600, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 400, maxX: 800, color: 'green' as const },
      { id: 2, x: 1200, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: -1, minX: 1000, maxX: 1400, color: 'purple' as const },
    ],
    texts: []
  },
  {
    ...
    slimes: [
      { id: 1, x: 400, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 300, maxX: 600, color: 'green' as const },
      { id: 2, x: 900, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: -1, minX: 700, maxX: 1100, color: 'purple' as const },
      { id: 3, x: 1500, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 1300, maxX: 1700, color: 'green' as const },
      { id: 4, x: 2000, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: -1, minX: 1800, maxX: 2200, color: 'purple' as const }
    ],
    texts: []
  },
  {
    ...
    coins: [
      { id: 1, x: 220, y: 350, collected: false }, { id: 2, x: 420, y: 230, collected: false },
      { id: 3, x: 670, y: 300, collected: false }, { id: 4, x: 920, y: 190, collected: false },
      { id: 5, x: 1080, y: 360, collected: false }, { id: 6, x: 1370, y: 300, collected: false },
      { id: 7, x: 1620, y: 210, collected: false }, { id: 8, x: 1870, y: 330, collected: false },
      { id: 9, x: 2170, y: 250, collected: false }, { id: 10, x: 2470, y: 170, collected: false },
      { id: 11, x: 2770, y: 300, collected: false }, { id: 12, x: 3070, y: 230, collected: false },
    ],
    fruits: [
      { id: 1, x: 780, y: 420, collected: false }, { id: 2, x: 1550, y: 200, collected: false },
      { id: 3, x: 2350, y: 420, collected: false }, { id: 4, x: 3200, y: 250, collected: false },
    ],
    slimes: [
      { id: 1, x: 500, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 420, maxX: 620, color: 'green' as const },
      { id: 2, x: 950, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: -1, minX: 880, maxX: 1150, color: 'purple' as const },
      { id: 3, x: 380, y: 280 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 350, maxX: 480, color: 'purple' as const },
      { id: 4, x: 1500, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 1350, maxX: 1700, color: 'green' as const },
      { id: 5, x: 2000, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: -1, minX: 1900, maxX: 2250, color: 'purple' as const },
      { id: 6, x: 2650, y: 500 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 2550, maxX: 2850, color: 'green' as const },
      { id: 7, x: 2170, y: 300 - SLIME_FOOT_OFFSET, alive: true, direction: 1, minX: 2130, maxX: 2280, color: 'purple' as const },
    ],
    texts: []
  }
];
...
export function OttertaleGame({ onExitToLobby }: { onExitToLobby?: () => void }) {
  ...
  const [quiz, setQuiz] = useState<{ word: string, options: string[] } | null>(null);
  const [activeBuff, setActiveBuff] = useState<{type: 'speed' | 'invincibility' | null, until: number}>({ type: null, until: 0 });
  ...
  useEffect(() => {
    ...
    if (gameState === 'playing' && !isPaused && !isDead) {
      bgmRef.current.play().catch((e) => console.warn("Browser prevented audio autoplay:", e));
    } else {
      bgmRef.current.pause();
    }
  ...
  useEffect(() => {
    const loadEverything = async () => {
      try {
        const [knightTex, tilesetTex, coinTex, slimeGreenTex, slimePurpleTex, platTex, fruitTex] = await Promise.all([
        ...
        setAssets({
          ...
          slimeGreen: extractFramesTrimmed(slimeGreenTex, 24, 24, 4, 0, 0, 4, 11, 88, 61),
          slimePurple: extractFramesTrimmed(slimePurpleTex, 24, 24, 4, 0, 0, 4, 11, 88, 61),
  ...
  const handleCollectCoin = (id: number) => {
    ...
    setCoins(prevCoins => {
      ...
      setCollectedLetters(prevLetters => {
        const uncollectedIndices = stageConfig.word.split('').map((_: string, i: number) => i).filter((i: number) => !prevLetters.includes(i));
        if (uncollectedIndices.length > 0 && prevLetters.length < next.filter(c => c.collected).length) {
          const randomIdx = uncollectedIndices[Math.floor(Math.random() * uncollectedIndices.length)];
    ...
    setFruits(prev =>
      prev.map(fruit => {
        if (fruit.id === id && !fruit.collected) {
          const question = spellingWords[Math.floor(Math.random() * spellingWords.length)];
          const isFirstCorrect = Math.random() > 0.5;
          setQuiz({
            word: question.word,
            options: isFirstCorrect ? [question.word, question.wrong] : [question.wrong, question.word]
          });
  ...
  const handleKillSlime = (id: number) => {
    if (isDead || isPaused) return;
    setSlimes(prev => prev.map(s => s.id === id ? { ...s, alive: false } : s));
    setScore(s => s + 2);
    playSFX('explosion'); 
  };
  ...
  const handleUpdateSlimePos = (id: number, newX: number, newDir: number) => {
    if (isPaused) return;
    setSlimes(prev => prev.map(s => s.id === id ? { ...s, x: newX, direction: newDir } : s));
  };

  const handlePlayerMove = (playerX: number) => {
    let targetCamX = playerX - 400;
    if (targetCamX < 0) targetCamX = 0;
    if (targetCamX > stageConfig.worldWidth - 800) targetCamX = stageConfig.worldWidth - 800;
  ...
  return (
    <div className="game-two-ottertale">
      ...
      <div className="game-wrapper">
        <Application width={800} height={600} background={0x808080}>
          <pixiContainer x={-cameraX} y={0}>
            <pixiTilingSprite texture={assets.groundTop} x={0} y={500} width={stageConfig.worldWidth} height={16} tileScale={{ x: 3, y: 3 }} />
            <pixiTilingSprite texture={assets.groundDirt} x={0} y={516} width={stageConfig.worldWidth} height={84} tileScale={{ x: 3, y: 3 }} />

            {/* --- Tutorial Tooltips Render Loop --- */}
            {stageConfig.texts && stageConfig.texts.map((t: any, i: number) => (
              <pixiText
                key={`text-${i}`}
                text={t.text}
                x={t.x}
            ...
            {stageConfig.platforms.map((plat: any, i: number) => (
              <pixiSprite key={i} texture={assets.platform} x={plat.left} y={plat.top} scale={{ x: 3, y: 3 }} />
            ))}

            <FinishLine x={stageConfig.finishLineX} y={200} />

            {coins.map((coin: any) => (
              <Coin key={coin.id} textures={assets.coin} x={coin.x} y={coin.y} collected={coin.collected} onCollect={() => handleCollectCoin(coin.id)} />
            ))}
            ...
            {slimes.map((slime: any) => (
              <Slime
                key={slime.id}
                textures={slime.color === 'purple' ? assets.slimePurple : assets.slimeGreen}
                slime={slime}
              ...
              return (
                <pixiText
                  ...
                  style={new TextStyle({
                    ...
                    stroke: { color: '#000000', width: 4 },
                    dropShadow: { color: '#000000', alpha: 1, blur: 0, distance: 3 }
                  })}
                />
              );
            })}
      ...
      {gameState === 'playing' && !isDead && !quiz && !isMenuPaused && !hasWon && (
        <button 
          className="retro-btn"
          onClick={() => { playSFX('tap'); setIsMenuPaused(true); }}
          style={{
            position: 'fixed', top: '20px', right: '20px', padding: '10px 16px',
            fontSize: '12px', backgroundColor: '#333',
      ...
      {gameState === 'playing' && !isPaused && !isDead && !hasWon && (
        <div className="controls-container">
          <div className="control-group">
            <MobileButton onDown={() => setVirtualKey('ArrowLeft', true)} onUp={() => setVirtualKey('ArrowLeft', false)} label="<" />
            <MobileButton onDown={() => setVirtualKey('ArrowRight', true)} onUp={() => setVirtualKey('ArrowRight', false)} label=">" />
          </div>
          <div className="control-group">
            <MobileButton onDown={() => setVirtualKey('ArrowUp', true)} onUp={() => setVirtualKey('ArrowUp', false)} label="^" />
          </div>
        </div>
      )}
      ...
      {gameState === 'lobby' && (
        <div style={{
          ...
          zIndex: 200, textAlign: 'center', padding: '20px'
        }}>
          <h1 style={{ fontSize: '32px', marginBottom: '40px', textShadow: '4px 4px 0px #000', lineHeight: '1.5' }}>OtterTale</h1>
          <div style={{ display: 'flex', gap: '20px', flexDirection: 'column' }}>
            <button
              className="retro-btn"
              onClick={() => { playSFX('tap'); setGameState('stageSelect'); }}
              style={{ padding: '16px 24px', fontSize: '16px', backgroundColor: '#4CAF50', color: 'white', cursor: 'pointer' }}
            >
              PLAY GAME
            </button>
            <button
              className="retro-btn"
              onClick={() => { playSFX('tap'); handleStartStage(TUTORIAL_STAGE); }}
              style={{ padding: '16px 24px', fontSize: '16px', backgroundColor: '#2196F3', color: 'white', cursor: 'pointer' }}
            >
              TUTORIAL
            </button>
            {/* 
            ...
            */}
            <button
              className="retro-btn"
              onClick={() => { playSFX('tap'); console.log('Exit game triggered (Placeholder)'); }}
              style={{ padding: '16px 24px', fontSize: '16px', backgroundColor: '#9e9e9e', color: 'white', cursor: 'pointer' }}
            >
              EXIT
            </button>
          </div>
      ...
      {gameState === 'stageSelect' && (
        <div style={{
          ...
          <h1 style={{ fontSize: '32px', marginBottom: '40px', textShadow: '4px 4px 0px #000' }}>SELECT STAGE</h1>
          <div style={{ display: 'flex', gap: '20px', flexDirection: 'column', marginBottom: '40px' }}>
            {STAGES.map((stage, index) => (
              <button
                key={stage.id}
                className="retro-btn"
                onClick={() => { playSFX('tap'); handleStartStage(STAGES[index]); }}
                style={{ padding: '16px 24px', fontSize: '16px', backgroundColor: '#9C27B0', color: 'white', cursor: 'pointer' }}
              >
          ...
          <button
            ...
            style={{ padding: '12px 20px', fontSize: '12px', backgroundColor: '#f44336', color: 'white', cursor: 'pointer' }}
          >
            BACK TO LOBBY
          </button>
        </div>
      ...
      {isMenuPaused && (
        <div style={{
          ...
          alignItems: 'center', justifyContent: 'center', color: '#ffffff', zIndex: 150
        }}>
          <h2 style={{ fontSize: '32px', marginBottom: '40px', textShadow: '4px 4px 0px #000' }}>PAUSED</h2>
          <div style={{ display: 'flex', gap: '20px', flexDirection: 'column' }}>
            <button className="retro-btn" onClick={() => { playSFX('tap'); setIsMenuPaused(false); }} style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#2196F3', color: 'white', cursor: 'pointer' }}>RESUME</button>
            <button className="retro-btn" onClick={() => handleExitToMenu('lobby')} style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#f44336', color: 'white', cursor: 'pointer' }}>QUIT</button>
          </div>
        </div>
      )}
      ...
      {quiz && (
        <div style={{
          ...
          <h2 style={{ fontSize: '20px', marginBottom: '40px', textShadow: '4px 4px 0px #000', textAlign: 'center', padding: '0 20px', lineHeight: '1.5' }}>WHAT IS THE CORRECT SPELLING?</h2>
          <div style={{ display: 'flex', gap: '20px', flexDirection: 'column' }}>
            {quiz.options.map((opt: string) => (
              <button 
                ...
                style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#2196F3', color: 'white', cursor: 'pointer' }}
              >
                {opt}
              </button>
            ))}
      ...
      {hasWon && (
        <div style={{
          position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh',
          backgroundColor: 'rgba(0, 100, 0, 0.9)', display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center', color: '#ffffff', zIndex: 200
        }}>
          <h1 style={{ fontSize: '40px', marginBottom: '20px', textShadow: '4px 4px 0px #000', textAlign: 'center' }}>{stageConfig.name === "TUTORIAL" ? "TUTORIAL COMPLETE!" : "LEVEL CLEARED!"}</h1>
          
          <div style={{ backgroundColor: '#111', padding: '20px', border: '4px solid #fff', boxShadow: '8px 8px 0px #000', textAlign: 'center', marginBottom: '40px', minWidth: '300px' }}>
            <h2 style={{ fontSize: '16px', margin: '0 0 20px 0', color: '#FFD700' }}>PERFORMANCE</h2>
            <p style={{ fontSize: '14px', margin: '10px 0' }}>SCORE: {score}</p>
            
            <div style={{ margin: '20px 0' }}>
              <p style={{ fontSize: '12px', marginBottom: '15px' }}>WORD PROGRESS:</p>
              <p style={{ fontSize: '16px', letterSpacing: '2px', color: '#aaa' }}>
                {stageConfig.word.split('').map((char: string, i: number) => collectedLetters.includes(i) ? char : '_').join('')}
              </p>
              
              {collectedLetters.length >= stageConfig.word.length ? (
                <p style={{ color: '#4CAF50', fontSize: '12px', marginTop: '20px' }}>PERFECT!</p>
              ) : (
                <p style={{ color: '#FF9800', fontSize: '12px', marginTop: '20px' }}>MISSED {stageConfig.word.length - collectedLetters.length} LETTERS</p>
              )}
            </div>
          </div>

          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center' }}>
            <button className="retro-btn" onClick={() => { playSFX('tap'); handleStartStage(stageConfig); }} style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#2196F3', color: 'white', cursor: 'pointer' }}>PLAY AGAIN</button>
            <button className="retro-btn" onClick={() => handleExitToMenu('stageSelect')} style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#9C27B0', color: 'white', cursor: 'pointer' }}>STAGES</button>
            <button className="retro-btn" onClick={() => handleExitToMenu('lobby')} style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#f44336', color: 'white', cursor: 'pointer' }}>LOBBY</button>
          </div>
        </div>
      )}
      ...
      {isDead && (
        <div style={{
          position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh',
          backgroundColor: '#8b0000', display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center', color: '#ffffff', zIndex: 200
        }}>
          <h1 style={{ fontSize: '48px', marginBottom: '20px', textShadow: '4px 4px 0px #000', textAlign: 'center' }}>YOU LOSE!</h1>
          <p style={{ fontSize: '16px', marginBottom: '40px' }}>SCORE: {score}</p>
          <div style={{ display: 'flex', gap: '20px', flexDirection: 'column' }}>
            <button className="retro-btn" onClick={() => { playSFX('tap'); handleStartStage(stageConfig); }} style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#4CAF50', color: 'white', cursor: 'pointer' }}>RETRY</button>
            <button className="retro-btn" onClick={() => handleExitToMenu('lobby')} style={{ padding: '16px 24px', fontSize: '14px', backgroundColor: '#f44336', color: 'white', cursor: 'pointer' }}>LOBBY</button>
          </div>
        </div>
      )}
