...
describe("bilingual ReaDirect content", () => {
  ...
  it("provides complete Filipino mission content with the same stable gameplay IDs", () => {
    ...
    english.forEach((mission, missionIndex) => {
      ...
      expect(translated.reading.pages).toHaveLength(
        mission.reading.pages.length,
      );
      expect(translated.action.correctChoiceId).toBe(
        mission.action.correctChoiceId,
      );
      expect(translated.action.choices.map(({ id }) => id)).toEqual(
        mission.action.choices.map(({ id }) => id),
      );
      expect(translated.questions.map(({ id }) => id)).toEqual(
        mission.questions.map(({ id }) => id),
      );
      translated.questions.forEach((question, questionIndex) => {
        ...
        expect(question.choices.map(({ id }) => id)).toEqual(
  ...
  it("uses short questions and simple modern Filipino words", () => {
    const formalWords =
      /\b(liwasan|hudyat|rehas|paunawa|panlabas|patungo|mantel|tala|palatandaan|paglalakbay|panganib|kagamitan|tuntunin|hinuha)\b/i;

    for (const mission of getMissions("fil")) {
      for (const question of mission.questions) {
        expect(question.prompt.trim().split(/\s+/).length).toBeLessThanOrEqual(
          7,
        );
        expect(
          [question.prompt, ...question.choices.map(({ text }) => text)].join(
            " ",
          ),
        ).not.toMatch(formalWords);
      }
    }
  });
  ...
  it("changes language without changing the active item, choice order, attempts, or answer", () => {
    ...
    state = missionReducer(state, {
      ...
      choiceId: getMission("plaza-welcome").action.correctChoiceId,
    });
    ...
    const idsBefore = state.round.questions.map(({ id, choices }) => ({
      id,
      choices: choices.map((choice) => choice.id),
    }));
    const attemptsBefore = state.attemptsByQuestion;
    const switched = missionReducer(state, {
      type: "SET_LANGUAGE",
      language: "fil",
    });
    ...
    expect(
      switched.round.questions.map(({ id, choices }) => ({
        id,
        choices: choices.map((choice) => choice.id),
      })),
    ).toEqual(idsBefore);
    expect(switched.round.questions[0].prompt).not.toBe(question.prompt);
    expect(switched.round.questions[0].correctChoiceId).toBe(
      question.correctChoiceId,
    );
  });

  it("restores the selected language with mission progress", () => {
    const englishRounds = createMissionRounds(
      getMissions("en"),
      createSeededRandom(4),
    );
    const filipinoRounds = createMissionRounds(
      getMissions("fil"),
      createSeededRandom(4),
    );
