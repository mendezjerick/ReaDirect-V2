...
describe("focused tutorial overlay", () => {
  ...
  it("points the spotlight and original cursor at the required control", async () => {
    ...
    render(
      <>
        <nav className="movement-controls">
          <button>Move</button>
        </nav>
  ...
  it("keeps the target and dialogue positioned apart after resize", async () => {
    vi.spyOn(HTMLElement.prototype, "getBoundingClientRect").mockImplementation(
      () =>
        window.innerHeight <= 390
          ? new DOMRect(10, 110, 140, 100)
          : new DOMRect(10, 520, 140, 100),
    );
    render(
      <>
        <nav className="movement-controls">
    ...
    // Short landscape viewports keep narration below the playfield.
    await waitFor(() =>
      expect(document.querySelector(".tutorial-narrator")).toHaveClass(
        "is-bottom",
      ),
    );
  });
  ...
  it("uses the interaction button only after it becomes available", () => {
    ...
    expect(tutorialTarget("interaction", true)).toBe(
      ".mission-interact-button",
    );
  });
  ...
  it("does not show unavailable narration controls in the compact tutorial", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /Narration unavailable/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/No approved narration recording/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("uses the activity intro as a responsive fallback before Answer Later appears", () => {
    expect(tutorialTarget("answerLater", false)).toContain(
      ".activity-intro-panel",
    );
  });

  it("retargets Answer Later to its confirmation button", () => {
    expect(tutorialTarget("answerLater", false)).toContain(
      '[data-tutorial="save-for-later"]',
    );
  });

  it("targets Continue to Questions after the mission choice is correct", () => {
    expect(tutorialTarget("continueQuestions", false)).toBe(
      '[data-tutorial="continue-questions"]',
    );
  });
  ...
  it("keeps the final Start Adventure panel centered without targeting itself", () => {
    ...
    expect(
      document.querySelector(".tutorial-spotlight"),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("places choice help outside the full activity panel", async () => {
    ...
    vi.spyOn(HTMLElement.prototype, "getBoundingClientRect").mockImplementation(
      function (this: HTMLElement) {
        if (this.classList.contains("answer-choice"))
          return new DOMRect(330, 390, 620, 82);
        if (this.getAttribute("role") === "dialog")
          return new DOMRect(320, 150, 1280, 690);
    ...
    expect(
      Number.parseFloat(narrator?.style.left ?? "0"),
    ).toBeGreaterThanOrEqual(1612);
  });
  ...
  it("shows a confirmation before skipping", async () => {
    ...
    view.rerender(
      <TutorialOverlay
        {...props({ ...tutorialState("movement"), skipConfirmationOpen: true })}
      />,
    );
    expect(
      screen.getByRole("alertdialog", { name: /Skip the tutorial/i }),
    ).toBeVisible();
    expect(
      screen.getByRole("button", { name: /Keep Learning/i }),
    ).toBeVisible();
  });
});
