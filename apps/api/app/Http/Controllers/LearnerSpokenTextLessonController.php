...
use App\Services\SpokenTextLessonSupportPresentation;
...
abstract class LearnerSpokenTextLessonController extends Controller
{
    public function __construct(
        ...
        protected readonly LessonTeachingStateMachine $teaching,
        protected readonly SpokenTextLessonSupportPresentation $supportPresentation,
        protected readonly TranscriptAlignmentService $transcriptAlignment,
        protected readonly LessonPracticeTryService $practiceTries,
    ) {}

    abstract protected function lessonNumber(): int;

    abstract protected function contentSnapshot(int $learnerId): array;

    abstract protected function missionTitle(): string;

    abstract protected function itemPresentation(): string;

    abstract protected function resultSegmentLabel(): string;

    abstract protected function achievementKey(): string;

    abstract protected function achievementName(): string;

    protected function lessonKey(): string
    {
        return "required-lesson-{$this->lessonNumber()}";
    }
    ...
    /** @param array<string, mixed>|null $alignment */
    private function diagnosisKey(
        string $classification,
        ?array $alignment,
    ): string {
        return match ($classification) {
            LessonTeachingStateMachine::CLASS_CLEAR_CORRECT => 'correct_spoken_text',
            LessonTeachingStateMachine::CLASS_CLEAR_INCORRECT => (string) (
                $alignment['diagnosis_key'] ?? 'different_clear_spoken_text'
            ),
    ...
    private function completion(LessonRun $run): ?array
    {
        ...
        $labels = [
            'mission-1' => $this->resultSegmentLabel(),
        ];
        $segments = collect(['mission-1'])->map(
