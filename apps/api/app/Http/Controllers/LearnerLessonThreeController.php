...
final class LearnerLessonThreeController extends Controller
{
    private const LESSON_KEY = 'required-lesson-3';

    private const MISSIONS = ['mission-1'];
    ...
    public function __construct(
        ...
        private readonly LessonThreeSupportPresentation $supportPresentation,
        private readonly TranscriptAlignmentService $transcriptAlignment,
    ...
    public function start(Request $request): JsonResponse
    {
        ...
        if ($progress?->stage === 'required_lessons'
            ...
            abort_unless(
                $completedRun,
                409,
                'Lesson 3 is not the learner\'s current required lesson.',
            );
        ...
        $run = DB::transaction(function () use ($session): LessonRun {
            ...
            return LessonRun::query()->create([
                ...
                'content_snapshot' => $this->content->lessonThreeSnapshot(
                    $session->learner_id,
                ),
            ]);
        });
    ...
    private function diagnosisKey(
        ...
        return match ($classification) {
            LessonTeachingStateMachine::CLASS_CLEAR_CORRECT => 'correct_phrase',
            LessonTeachingStateMachine::CLASS_CLEAR_INCORRECT => (string) (
                $alignment['diagnosis_key'] ?? 'different_clear_phrase'
            ),
