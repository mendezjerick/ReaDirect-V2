...
final class LessonThreeSupportPresentation
{
    ...
    public function forCurrentItem(
        ...
        if ($run->status === LessonRun::STATUS_COMPLETED || $item === null) {
            return $this->presentation(
                ...
                [['kind' => 'published', 'speech_key' => 'lesson-3-complete']],
                'completion',
                self::AFTER_NONE,
            );
        }
        ...
        if ($response === null) {
            ...
            $speechKey = $position === 1
                ? 'lesson-3-mission-1'
                : "lesson-3-mission-1-item-{$position}";
        ...
        if ($response->teaching_state === LessonTeachingStateMachine::STATE_GIVING_CLUE) {
            return $this->presentation(
                ...
                [
                    ...
                    [
                        'kind' => 'published',
                        'speech_key' => 'lesson-3-clue-mission-1',
                    ],
                ],
                'clue',
        ...
        if ($latestAttempt?->attempt_kind === LessonItemAttempt::KIND_TECHNICAL) {
            return $this->presentation(
                ...
                [[
                    'kind' => 'published',
                    'speech_key' => 'lesson-3-technical-retry',
                ]],
                'technical_retry',
                self::AFTER_RECORD,
            );
    ...
    private function terminalPresentation(
        ...
        $speechKey = match ($response->outcome) {
            LessonTeachingStateMachine::OUTCOME_INDEPENDENT_CORRECT => 'lesson-3-feedback-independent',
            LessonTeachingStateMachine::OUTCOME_SUPPORTED_CORRECT => 'lesson-3-feedback-supported',
            LessonTeachingStateMachine::OUTCOME_DEMONSTRATED => 'lesson-3-feedback-demonstrated',
            LessonTeachingStateMachine::OUTCOME_NOT_YET_CORRECT => 'lesson-3-feedback-not-yet',
            LessonTeachingStateMachine::OUTCOME_UNSCORABLE_AUDIO => 'lesson-3-feedback-unscorable',
            default => null,
        };
