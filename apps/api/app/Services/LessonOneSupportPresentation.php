...
final class LessonOneSupportPresentation
{
    ...
    public function forCurrentItem(
        ...
        if ($run->status === LessonRun::STATUS_COMPLETED || $item === null) {
            return $this->presentation(
                ...
                [['kind' => 'published', 'speech_key' => 'lesson-1-complete']],
                'completion',
                self::AFTER_NONE,
            );
        }
        ...
        if ($response === null) {
            ...
            $speechKey = $position === 1
                ? "lesson-1-{$run->mission_key}"
                : "lesson-1-{$run->mission_key}-item-{$position}";
        ...
        if ($response->teaching_state === LessonTeachingStateMachine::STATE_DEMONSTRATING) {
            $letter = strtoupper((string) ($item['spoken_target'] ?? $item['uppercase_form'] ?? ''));

            return $this->presentation(
                $sequenceKey,
                [[
                    'kind' => 'published',
                    'speech_key' => "lesson-1-letter-demo-{$letter}",
                ]],
                'demonstration',
                self::AFTER_CONTINUE_SUPPORT,
            );
        ...
        if ($latestAttempt?->attempt_kind === LessonItemAttempt::KIND_TECHNICAL) {
            return $this->presentation(
                ...
                [[
                    'kind' => 'published',
                    'speech_key' => 'lesson-1-technical-retry',
                ]],
                'technical_retry',
                self::AFTER_RECORD,
            );
    ...
    private function terminalPresentation(string $sequenceKey, LessonResponse $response): array
    {
        ...
        $speechKey = match ($response->outcome) {
            ...
            LessonTeachingStateMachine::OUTCOME_SUPPORTED_CORRECT => 'lesson-1-feedback-supported',
            LessonTeachingStateMachine::OUTCOME_DEMONSTRATED => 'lesson-1-feedback-demonstrated',
            LessonTeachingStateMachine::OUTCOME_NOT_YET_CORRECT => 'lesson-1-feedback-not-yet',
            LessonTeachingStateMachine::OUTCOME_UNSCORABLE_AUDIO => 'lesson-1-feedback-unscorable',
            default => null,
        };
