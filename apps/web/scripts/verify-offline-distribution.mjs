...
export function resolveDistributionPaths({ repositoryRoot, webRoot }) {
  const outputDirectory = path.join(repositoryRoot, "output", "releases");
  return resolveOfflineAndroidRelease({
    gradlePath: path.join(webRoot, "android-apk", "app", "build.gradle"),
    outputDirectory,
  });
}

const { signedAab, localTestingApks } = resolveDistributionPaths({
  repositoryRoot,
  webRoot,
});
...
function verifyOfflineDistribution() {
  ...
  if (assetPackEntries.length === 0) {
    throw new Error(
      "Bundletool output does not contain the offline model pack.",
    );
  }
...
if (invokedAsScript) verifyOfflineDistribution();
