...
export const CLARA_LINK_START_ROUTE_SWAP_MS = 2600;
export const CLARA_LINK_START_DURATION_MS = 3000;
...
function resolveTransitionColors(element: HTMLElement): LinkStartColors | null {
  ...
  return { core: values[0], cover: values[1], streaks: values.slice(2) };
}
  ...
  if (coverProgress > 0) {
    const revealProgress = clamp(
      (elapsed - LINK_START_REVEAL_START_MS) /
        (CLARA_LINK_START_DURATION_MS - LINK_START_REVEAL_START_MS),
    );
...
export function ClaraLinkStartTransition() {
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      drawFrame(context, metrics, streaks, colors, elapsed);
      if (elapsed < CLARA_LINK_START_DURATION_MS) {
  ...
  return (
    <div
      className="route-transition route-transition--clara"
